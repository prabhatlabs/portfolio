...
export function render(): string {
    ...
    for (const line of cleanDescription(myInfo.description).split("\n")) {
        ...
        for (const p of line.trim().match(/.{1,76}(?:\s|$)/g) || [
            line.trim(),
        ]) {
    ...
    for (const [i, p] of visibleProjects.entries()) {
        out.push(
            `echo "  \${GREEN}${i + 1}.\${RESET} \${BOLD}${p.title}\${RESET}"`,
        );
        ...
        if (p.skills?.length) {
            const stack = p.skills
                .map((s) => `\${YELLOW}${s.name}\${RESET}`)
                .join(" ");
            out.push(`echo "  \${DIM}Stack:\${RESET} ${stack}"`);
        }
        if (p.links?.length) {
            const links = p.links
                .map((l) => `\${BLUE}${l.name}\${RESET}: ${l.url}`)
                .join("  ");
    ...
    for (const e of experiences) {
        ...
        out.push(
            `echo "  \${BOLD}${e.company.padEnd(26)}\${RESET}\${DIM}${e.period ?? ""}\${RESET}"`,
        );
        out.push(
            `echo "  \${MAGENTA}${e.position.padEnd(24)}\${RESET}\${DIM}${e.location ?? ""}\${RESET}"`,
        );
        if (pills?.length) {
            const names = (pills as { name: string }[])
                .map((s) => `\${DIM}${s.name}\${RESET}`)
                .join(" ");
            out.push(`echo "  ${names}"`);
        }
        out.push('echo ""');

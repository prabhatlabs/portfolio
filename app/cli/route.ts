import { theme, myInfo, experiences, visibleProjects, groupedSkills, cleanDescription } from './lib';
...
function render(): string {
  ...
  for (const line of cleanDescription(myInfo.description).split('\n')) {
    ...
    for (const part of line.trim().match(/.{1,76}(?:\s|$)/g) || [line.trim()]) {
      out.push(`  ${part.trim()}`);
    }
  }
  out.push('');
  ...
  for (const [i, p] of visibleProjects.entries()) {
    ...
    if (p.skills?.length) {
      const stack = p.skills.map(s => `${theme.accent(s.name)}`).join(' ');
      out.push(`  ${theme.muted('Stack:')} ${stack}`);
    }
    if (p.links?.length) {
      const links = p.links.map(l => `${theme.link(l.name)}: ${l.url}`).join('  ');
  ...
  for (const e of experiences) {
    ...
    out.push(`  ${theme.bold(e.company.padEnd(26))}${theme.muted(e.period ?? '')}`);
    out.push(`  ${theme.highlight(e.position.padEnd(24))}${theme.muted(e.location ?? '')}`);
    if (pills?.length) {
      const names = (pills as { name: string }[]).map(s => `${theme.muted(s.name)}`).join(' ');
      out.push(`  ${names}`);
    }
    out.push('');
  }
  ...
  for (const link of myInfo.contacts) out.push(`  ${theme.primary(link.name.padEnd(14))} ${link.url}`);
  out.push('');
